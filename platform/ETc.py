...
def saveETc():
    ...
    try:
        ...
        json_crops = db.getAllCrops()
        json_crops = ast.literal_eval(json_crops)
    except:
        print("cannot retrieve Evapotranspiration or json file with crops info from database")
    #at this point we need to evaluate etc.

    for crop in json_crops:
        try:

            print(crop)
            crop_id = crop['id']
            seeding_date = crop['dateseeded']

            seeding_date = datetime.strptime(seeding_date, "%Y-%m-%d %H:%M:%S.%f")
            seeding_date = seeding_date.date()
        ...
        try:
            path_json_file = currentdir +'/' + crop_type +'.json'
        ...
        try:
            ...
            print("cannot save the json file with trigger information")
